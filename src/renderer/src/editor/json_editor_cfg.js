...
const validGuilds = ['KM', 'OS', 'SGW', 'GL', 'KG', 'CKN', 'OHM', 'MC', 'WKS', 'RA', 'SC', 'ZT', 'PE', 'KGKS', 'ES', 'GP', 'ZH', 'ZS', 'NPC', 'OK', 'KS', 'LE']
...
JSONEditor.defaults.editors.filePath = class FilePathEditor extends JSONEditor.defaults.editors.string {
  ...
  clickHandler(e) {
    ...
    //     this.setValue(result.filePaths[0].replaceAll('\\', '/'))
    //   }
    // })
    console.log(e)
  }
...
JSONEditor.defaults.custom_validators.push((schema, value, path) => {
  const errors = []
  if (schema.format === 'textarea' && true) {
    let obj
    try {
      obj = JSON.parse(value)
      if (schema.contentType === 'guild' && true && value !== '') {
        obj.forEach(val => {
