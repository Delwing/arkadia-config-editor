...
class ConfigLoader {
    ...
    constructor(path, name) {
        this.path = path
        this.name = name
        this.configPath = `${this.path}/${this.name}.json`
    ...
    load() {
        ...
        Array.from(keys).sort().forEach(key => {
            ...
            listElement.addEventListener("click", function () {
                let elmnt = document.body.querySelector("[data-schemapath=\"root." + key + "\"]")
        ...
        for (const key in readmeParsed) {
            if (readmeParsed.hasOwnProperty(key)) {
                ...
                let selector = document.body.querySelector("[data-schemapath=\"root." + key + "\"]")
                if (selector !== null) {
                    selector.querySelector("small").innerHTML = converter.makeHtml(element)
                } else {
        ...
        document.querySelector(".save-button").addEventListener("click", function (e) {
            ...
            setTimeout(function(){ toast.className = toast.className.replace("visible", "").trim(); }, 3500);
        })
    }

    save() {
        let value = {};
        Object.assign(value, this.editor.getValue())
